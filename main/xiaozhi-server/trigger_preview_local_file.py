#!/usr/bin/env python3
"""
Preview a local image file on Xiaozhi screen through app.py HTTP endpoints.

Functions:
  - list_sessions(base_url)
  - trigger_preview_local_file(base_url, file_path, ...)

Examples:
  python trigger_preview_local_file.py --server http://127.0.0.1:8003 --list-sessions
  python trigger_preview_local_file.py --server http://127.0.0.1:8003 --device-id 94:a9:90:28:e8:ec --file-path C:\\tmp\\demo.jpg
"""
...
def trigger_preview_local_file(
    base_url: str,
    *,
    file_path: str,
    session_id: str = "",
    device_id: str = "",
    tool_name: str = "self.screen.preview_image",
    tool_timeout: int = 90,
    ...
    url = f"{base_url.rstrip('/')}/mcp/device/preview_local_file"
    return _request_json("POST", url, payload=payload, timeout=request_timeout)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Preview a local image file on Xiaozhi screen"
    )
    parser.add_argument(
        "--server",
    ...
    parser.add_argument(
        "--file-path",
        default="",
        help="Local image file path on server machine",
    )
    parser.add_argument(
        "--tool-name",
        default="self.screen.preview_image",
...
def main() -> int:
    ...
    try:
        result = trigger_preview_local_file(
            args.server,
